...
import io.vacivor.nexo.dal.repository.OAuth2RefreshTokenRepository;
...
@Singleton
@Primary
public class DatabaseOAuth2RefreshTokenStore implements OAuth2RefreshTokenStore {

  private final OAuth2RefreshTokenRepository repository;

  public DatabaseOAuth2RefreshTokenStore(OAuth2RefreshTokenRepository repository) {
    this.repository = repository;
  }

  @Override
  public void store(OAuth2RefreshToken token) {
    OAuth2RefreshTokenEntity entity = repository.findByToken(token.getToken()).orElseGet(OAuth2RefreshTokenEntity::new);
  ...
  public RefreshTokenConsumeResult<OAuth2RefreshToken> consumeWithStatus(String token) {
    Optional<OAuth2RefreshTokenEntity> entityOpt = repository.findByToken(token);
    if (entityOpt.isEmpty()) {
      return RefreshTokenConsumeResult.status(RefreshTokenConsumeStatus.NOT_FOUND);
    }
    OAuth2RefreshTokenEntity entity = entityOpt.get();
    if (entity.isRevoked()) {
  ...
  public void revokeFamily(String familyId) {
    ...
    for (OAuth2RefreshTokenEntity entity : repository.findByFamilyId(familyId.trim())) {
      if (!entity.isRevoked()) {
        entity.setRevoked(true);
        repository.update(entity);
      }
  ...
  private OAuth2RefreshToken toToken(OAuth2RefreshTokenEntity entity) {
