...
public class RedisSessionExpirationListener {

  private static final Logger LOG = LoggerFactory.getLogger(RedisSessionExpirationListener.class);
  ...
  void start() {
    ...
    try {
      pubSubConnection.sync().configSet("notify-keyspace-events", "Egx");
    } catch (Exception e) {
      LOG.warn("Failed to configure Redis keyspace notifications via CONFIG SET", e);
    }
    ...
    LOG.info("Subscribed Redis session listener to keyspace pattern {} and created pattern {}",
